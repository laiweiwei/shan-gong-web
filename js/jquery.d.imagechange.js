...
jQuery.fn.extend({
	...
	d_imagechange:function(setting){
		...
		var config = $.extend({
			...
			desc:true,						// 是否有描述
			btn:true,						// 是否显示按钮
			repeat:'no-repeat',				// 重复规则 'no-repeat' 'repeat-x'
											// 'repeat-y' 'repeat'
			...
			animateStyle:'o'				// 动画效果:'o':渐显 'x':横向滚动 'y':纵向滚动
											// 'show':原地收缩伸展 'show-x':横向收缩伸展
											// 'show-y':纵向收缩伸展' none':无动画
		...
		return $(this).each(function(){
			...
			// 插入空div修正IE的绝对定位BUG
			$('<div style=\"height:0px;\" />').appendTo(_this);
